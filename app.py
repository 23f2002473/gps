...
current_navigation = {
    ...
    "started_at": None,
    "completed_steps": [],
    "destination_version": 1  # Track which destination version
}
...
# Add version tracking - increments when destination changes
...
def reset_navigation_state():
    """Reset navigation state - called when destination changes"""
    global current_navigation, active_step, completed_steps
    
    print("\n🔄 RESETTING NAVIGATION STATE")
    print("   Reason: Destination changed")
    print("   Clearing: active navigation, steps, progress")
    
    current_navigation = {
        "is_active": False,
        "destination": None,
        "user_location": current_navigation.get("user_location"),  # Keep location
        "current_step": 0,
        "total_steps": 0,
        "started_at": None,
        "completed_steps": [],
    ...
    print("✅ Navigation state reset complete\n")
...
def get_current_destination():
    ...
    try:
        ...
        print(f"📍 Destination requested: {destination['name']}")
        print(f"🎯 Distance: {destination.get('calculated_distance', destination['distance'])}")
        print(f"🔢 Version: {destination.get('version', 1)}")
        
        return jsonify({
            "success": True,
            "destination": destination,
            "auto_start": True,
            "version": destination_version,  # Include current version
            "timestamp": datetime.datetime.now().isoformat()
        }), 200
...
@app.route('/api/destination/update', methods=['POST'])
def update_destination():
    """Update the current destination - RESETS NAVIGATION if destination changed"""
    try:
        data = request.get_json()
        ...
        global CURRENT_DESTINATION, destination_version
        
        # Check if destination actually changed
        destination_changed = False
        old_destination_name = CURRENT_DESTINATION.get('name')
        
        # Check name change
        if 'name' in data and data['name'] != CURRENT_DESTINATION.get('name'):
            destination_changed = True
            print(f"🔄 Destination name changed: {old_destination_name} → {data['name']}")
        
        # Check coordinates change
        if 'coordinates' in data:
            old_coords = CURRENT_DESTINATION.get('coordinates', {})
            new_coords = data['coordinates']
            if (old_coords.get('latitude') != new_coords.get('latitude') or 
                old_coords.get('longitude') != new_coords.get('longitude')):
                destination_changed = True
                print(f"🔄 Destination coordinates changed")
        
        # If destination changed, increment version and reset navigation
        if destination_changed:
            destination_version += 1
            print(f"\n🔢 VERSION INCREMENTED: {destination_version - 1} → {destination_version}")
            
            # IMPORTANT: Reset all navigation state
        ...
        if 'description' in data:
            CURRENT_DESTINATION['description'] = data['description']
        
        # Update version and timestamp
        CURRENT_DESTINATION['version'] = destination_version
        CURRENT_DESTINATION['updated_at'] = datetime.datetime.now().isoformat()
        
        print(f"✅ Destination updated: {CURRENT_DESTINATION['name']}")
        print(f"🔢 Current version: {destination_version}")
        ...
        return jsonify({
            ...
            "navigation_reset": destination_changed,  # Tell mobile app navigation was reset
            "timestamp": datetime.datetime.now().isoformat()
        }), 200
...
@app.route('/api/destination/check-version', methods=['GET'])
def check_destination_version():
    """Check if destination version changed (for mobile app to detect changes)"""
    try:
        # Mobile app sends its current version
        client_version = request.args.get('version', type=int, default=0)
        
        # Check if server version is different
        destination_changed = client_version != destination_version
        
        response = {
            "success": True,
            "server_version": destination_version,
            "client_version": client_version,
            "destination_changed": destination_changed,
            "current_destination_name": CURRENT_DESTINATION['name']
        }
        
        if destination_changed:
            response["message"] = "Destination has changed. Please reload navigation."
            response["action_required"] = "refresh_destination"
            print(f"⚠️  Version mismatch detected: Client={client_version}, Server={destination_version}")
...
def update_location():
    ...
    try:
        ...
        response_data = {
            ...
            "received_at": location_record["server_received_at"],
            "total_updates": len(location_updates),
            "destination_version": destination_version  # Include version so mobile app can detect changes
        }
...
def start_navigation():
    ...
    try:
        ...
        current_navigation = {
            ...
            "started_at": datetime.datetime.now().isoformat(),
            "completed_steps": [],
            "destination_version": destination_version  # Lock to current version
        }
        
        print(f"🚀 NAVIGATION STARTED")
        print(f"🎯 Destination: {CURRENT_DESTINATION['name']}")
        print(f"🔢 Locked to version: {destination_version}")
        print(f"📍 From: {data.get('user_location')}")
        print(f"📊 Route: {data.get('total_steps')} steps, {data.get('total_distance')}")
        print("=" * 60)
        ...
        return jsonify({
            ...
            "message": f"Navigation started to {CURRENT_DESTINATION['name']}",
            "destination_name": CURRENT_DESTINATION['name'],
            "destination_version": destination_version,  # Tell mobile app which version
            "voice_announcement": f"Navigation started. Proceeding to {CURRENT_DESTINATION['name']}. {data.get('total_distance')} ahead.",
            "timestamp": datetime.datetime.now().isoformat()
        }), 200
...
def get_navigation_status():
    ...
    try:
        return jsonify({
            ...
            "latest_location": location_updates[-1] if location_updates else None,
            "destination": CURRENT_DESTINATION,
            "destination_version": destination_version  # Include version
        }), 200
...
if __name__ == '__main__':
    ...
    print(f"🎯 Current destination: {CURRENT_DESTINATION['name']}")
    print(f"📍 Coordinates: {CURRENT_DESTINATION['coordinates']['latitude']}, {CURRENT_DESTINATION['coordinates']['longitude']}")
    print(f"🔢 Destination version: {destination_version}")
    print("📡 Ready for navigation requests...")
    print("\n🔗 API Endpoints:")
    print("\n📍 Destination:")
    print("   GET  /api/destination - Get current destination")
    print("   POST /api/destination/update - Update destination (auto-resets navigation)")
    print("   GET  /api/destination/check-version - Check if destination changed")
    print("\n📡 Location:")
    print("   POST /api/location/update - Update location")
    print("   GET  /api/location/current - Get current location")
    print("   GET  /api/location/history - Get location history")
    ...
    print("   GET  /api/health - Health check")
    print("\n✨ NEW FEATURES:")
    print("   • Automatic navigation reset when destination changes")
    print("   • Version tracking to detect destination changes")
    print("   • No need to refresh app manually!")
    print("=" * 60)
